...
import org.flybird.rock.mapper.UmsMemberMapper;
import org.flybird.rock.model.UmsMember;
import org.flybird.rock.model.UmsMemberExample;
import org.flybird.rock.model.UmsMemberLevel;
import org.flybird.rock.model.UmsMemberLevelExample;
import org.flybird.rock.portal.domain.MemberDetails;
import org.flybird.rock.portal.service.RedisService;
...
public class UmsMemberServiceImpl implements UmsMemberService {
    ...
    @Override
    public CommonResult update(UmsMember member){
        UmsMemberExample example = new UmsMemberExample();
        example.createCriteria().andIdEqualTo(member.getId());
        List<UmsMember> umsMembers = memberMapper.selectByExample(example);
        if (CollectionUtils.isEmpty(umsMembers)) {
            return CommonResult.failed("该用户不已经存在");
        }
        UmsMember umsMember = umsMembers.get(0);
        umsMember.setGender(member.getGender());;
        umsMember.setBirthday(member.getBirthday());;
        umsMember.setIcon(member.getIcon());;
