package org.flybird.rock.controller;

import org.flybird.rock.common.api.CommonPage;
import org.flybird.rock.common.api.CommonResult;
import org.flybird.rock.dto.UmsAdminLoginParam;
...
public class UmsAdminController {
    ...
    public CommonResult login(@RequestBody UmsAdminLoginParam umsAdminLoginParam, BindingResult result) {
        ...
        int ret = adminService.login(umsAdminLoginParam.getUsername(), umsAdminLoginParam.getPassword(),
                umsAdminLoginParam.getKey(), umsAdminLoginParam.getCode(), token);
        if(ret == -2) {
            return CommonResult.validateFailed("验证码错误！");
        } else if (ret == -1 || token == null || token.size() == 0){
            return CommonResult.validateFailed("用户名或者密码错误！");
        }

        Map<String, String> tokenMap = new HashMap<>();
        tokenMap.put("token", token.get(0));
